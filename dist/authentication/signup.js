...
function registerKey(keyCredentialObject, userId) {
    ...
    const attestation = CBOR.decodeFirstSync(Buffer.from(keyCredentialObject.attestationObject, 'base64'));
    const authenticatorData = (0, util_1.parseAuthenticatorData)(attestation.authData);
    if (process.env.RPID &&
    ...
    if (process.env.ALLOWEDALGORITHMS &&
        !process.env.ALLOWEDALGORITHMS.split(',').includes(authenticatorData.attestedCredentialData.credentialPublicKey.kty)) {
        return {
            status: 403,
    ...
    switch (attestation.fmt) {
        case 'packed':
            validAttestationSignature = (0, Packed_1.PackedVerify)(attestation, attestation.attStmt, clientDataHash, authenticatorData);
            break;
        case 'fido-u2f':
            validAttestationSignature = (0, FIDO_U2F_1.FIDOU2FVerify)(attestation, clientDataHash);
            break;
        case 'none':
