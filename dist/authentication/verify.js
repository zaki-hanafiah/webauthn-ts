...
function verify(assertion, userId) {
    ...
    const jwk = user.credentialPublicKey;
    const keyObject = crypto_1.default.createPublicKey(jwk);
    const sig = Buffer.from(assertion.response.signature, 'base64');
    const verify = user.credentialPublicKey.kty === 'RSA'
    ...
    if (!(authenticatorData.signCount > user.signCount)) {
        ...
        user.signCount = authenticatorData.signCount;
